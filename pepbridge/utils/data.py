...
from pepbridge.modules.protein.constants import PAD_RESIDUE_INDEX
...
class PaddingCollate(object):
    ...
    def __init__(self, length_ref_key='aa', pad_values=DEFAULT_PAD_VALUES, no_padding=DEFAULT_NO_PADDING, eight=True):
        ...
        self.padding_set_1 = PADDING_SET_1
        self.eight = eight
    ...
    def __call__(self, data_list):
        ...
        max_length_surf = max([data['surf_pos'].size(0) for data in data_list])
        keys = self._get_common_keys(data_list)

        # 'keys
        # {'pos_heavyatom', 'resseq', 'icode', 'torsion_angle', 'id', 'torsion_angle_mask', 'chain_nb', 'surf_pos',
        #  'pts_rec_mask_symmetric', 'chain_id', 'mask_heavyatom', 'aa', 'res_nb', 'generate_mask', 'surf_hbond',
        #  'surf_hp'}'

        if self.eight:
            max_length = math.ceil(max_length / 8) * 8
            max_length_surf = math.ceil(max_length_surf / 8) * 8
        data_list_padded = []
        ...
        for data in data_list:
            ...
            for k, v in data.items():
                if k in keys:
                    if (k not in self.no_padding) and (k not in self.padding_set_1):
                        data_padded[k] = self._pad_last(v, max_length, value=self._get_pad_value(k))
                    elif (k not in self.no_padding):
                        data_padded[k] = self._pad_last(v, max_length_surf, value=self._get_pad_value(k))
                    else:
                        data_padded[k] = v
