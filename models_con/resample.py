...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        ...
        # indices_np = np.random.choice(len(p)-1, size=(batch_size,), p=p[1:]/np.sum(p[1:])) + 1
    ...
    def weight_generation(self, time_step, batch_size, device):
        w = self.weights()
        p = w / np.sum(w)

        # Generate deterministic indices based on time_step
        indices_np = np.array([time_step for i in range(batch_size)])
