...
class SFEncoder(nn.Module):
    def __init__(self, encoder_conf):
        ...
        self.angle_net = nn.Sequential(
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, self._ipa_surf_bb_sfm_conf.c_s),nn.ReLU(),
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, self._ipa_surf_bb_sfm_conf.c_s),nn.ReLU(),
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, 5)
            # nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, 22)
        )

        # seq
        self.current_seq_embedder = nn.Embedding(22, self._ipa_surf_bb_sfm_conf.c_s)
        self.seq_net = nn.Sequential(
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, self._ipa_surf_bb_sfm_conf.c_s),nn.ReLU(),
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, self._ipa_surf_bb_sfm_conf.c_s),nn.ReLU(),
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, 20)
            # nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, 22)
        )

        # mixer
        self.res_feat_mixer = nn.Sequential(
            nn.Linear(3 * self._ipa_surf_bb_sfm_conf.c_s + self.angles_embedder.get_out_dim(in_dim=5), self._ipa_surf_bb_sfm_conf.c_s),
            nn.ReLU(),
            nn.Linear(self._ipa_surf_bb_sfm_conf.c_s, self._ipa_surf_bb_sfm_conf.c_s),
        )

        self.feat_dim = self._ipa_surf_bb_sfm_conf.c_s
        ...
        for b in range(self._ipa_surf_bb_sfm_conf.num_blocks):
            ...
            self.trunk[f'ipa_ln_{b}'] = nn.LayerNorm(self._ipa_surf_bb_sfm_conf.c_s)
            tfmr_in = self._ipa_surf_bb_sfm_conf.c_s
            tfmr_layer = torch.nn.TransformerEncoderLayer(
                d_model=tfmr_in,
                nhead=self._ipa_surf_bb_sfm_conf.seq_tfmr_num_heads,
            ...
            self.trunk[f'seq_tfmr_{b}'] = torch.nn.TransformerEncoder(
                tfmr_layer, self._ipa_surf_bb_sfm_conf.seq_tfmr_num_layers, enable_nested_tensor=False)
            ...
            if b < self._ipa_surf_bb_sfm_conf.num_blocks-1:
                ...
                edge_in = self._ipa_surf_bb_sfm_conf.c_z
                self.trunk[f'edge_transition_{b}'] = ipa_sfm.EdgeTransition(
                    node_embed_size=self._ipa_surf_bb_sfm_conf.c_s,
                    edge_embed_in=edge_in,
                    edge_embed_out=self._ipa_surf_bb_sfm_conf.c_z,
                )
