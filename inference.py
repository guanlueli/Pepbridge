...
base_dir = '.'
data_dir = '/data_surface/PepBridge'
...
if __name__ == '__main__':
    ...
    args.add_argument('--ckpt', type=str, default='./logs/learn_surf_angle/checkpoints/100000.pt')
    args.add_argument('--output', type=str, default='./pepbridge')
    args.add_argument('--num_steps', type=int, default=200)
    args.add_argument('--num_samples', type=int, default=10)
    args.add_argument('--sample_surf', type=bool, default=True)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        traj_1 = model.sample(batch,num_steps=parser.num_steps, sample_surf = parser.sample_surf,sample_bb=parser.sample_bb,sample_ang=parser.sample_ang,sample_seq=parser.sample_seq)
        ...
        torch.save(traj_1[-1],f'{save_dir}/{batch["id"][0]}.pt')
